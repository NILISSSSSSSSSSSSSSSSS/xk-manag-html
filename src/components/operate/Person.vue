                      ...
                      <el-row :gutter="10">
                          ...
                          <!-- <el-col :span="8">
                              <el-form-item label="反馈时间：">
                                <el-select v-model="searchForm.time" :size="$size" style="width: 100%" @change="getTimeScope">
                                  <el-option v-for="item in timeRange" :key="item" :label="item.label" :value="item.value"></el-option>
                                </el-select>
                              </el-form-item>
                          </el-col> -->
                      </el-row>
                  ...
                  <el-table :data="tableData" width="100%" stripe class="margintop20" v-loading="loading" @selection-change="selectionChange">
                      ...
                      <el-table-column  label="操作"  fixed="right">
                          <template slot-scope="scope">
                              ...
                              <!--<el-button type="text" :size="$size" @click="personDelete(scope.row.id,scope.row.userName)" v-permission="'feedbackDelete'">删除</el-button>-->
                          </template>
                      </el-table-column>
                  </el-table>
                  <pagination :currentPage="pageData.currentPage" :pageSize="pageData.pageSize" :total="pageData.total" @change="changePage"></pagination>
                  <el-dialog title="个人反馈详情" width="800px" :visible.sync="detailDialog" :before-close="detailClose">
...
export default {
  ...
  computed: {
      ...mapState({
          searchData: state => state.operatePlatform.personCommit
      })
  },
  methods: {
      ...mapMutations({
        setPersonCommit: "operatePlatform/setPersonCommit"
      }),
    resetForm() {
        this.setPersonCommit();
    ...
    getPersonalList() { // 个人反馈列表
        ...
        let paramList = {
            ...
            endTime: end/1000,
            lastUpdatedAt: this.searchForm.time,
            xkUserId: this.searchForm.xkUserId
        };
      console.log('paramList',paramList)
      this.loading = true;
      this.setPersonCommit(Object.assign({searchForm: this.searchForm}, {pageData: this.pageData}));
      this.$http.get(this.$api.personalFeedbackPage, paramList, true)
        .then(res => {
          this.loading = false;
    ...
    personDelete(id, name) { // 删除
        this.$confirm(`确定删除用户${name}的信息？`, "提示", {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        })
        .then(() => {
            this.$http.get(this.$api.personalFeedbackDelete, {ids: [id]}, true)
            .then(res => {
    ...
    batchDelete() { // 批量删除
        ...
        .then(() => {
            this.$http.get(this.$api.personalFeedbackDelete, {ids: this.ids}, true)
            .then(res => {
                this.$message.success("删除成功")
                this.getPersonalList();
            })
