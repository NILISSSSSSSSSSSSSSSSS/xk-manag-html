...
export default {
    ...
    watch: {
        nowCode(val) {
            this.nowUserCode = val
            this.getMemberReconmend();
        }
    },
    // computed: {
    //     ...mapState({
     ...
     changePage(page,pageSize) { // 分页
      ...
      getMembersList() { // 会员列表
        let paramList = {
          ...
          securityCode: this.$route.query.code // 推荐人的安全码
        }
